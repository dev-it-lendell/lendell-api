...
class Endorsement extends Model {
  static async select(
    ...
    try {
      let query = `SELECT 
          *
      FROM
          lendellp_losis_test_new.tbl_endo`; // Raw SQL query
  ...
  static async insert(data, transaction) {
    try {
      ...
      const query = `INSERT INTO lendellp_losis_test_new.tbl_endo (${fields}) VALUES (${values})`;
      ...
      const selectQuery = `SELECT * FROM lendellp_losis_test_new.tbl_endo WHERE id = :id`;
  ...
  static async update(
    ...
    try {
      ...
      const query = `UPDATE lendellp_losis_test_new.tbl_endorsemen SET ${setClause} WHERE ${whereClause}`;
      ...
      const selectQuery = `SELECT * FROM lendellp_losis_test_new.tbl_endo WHERE ${whereClause}`;
